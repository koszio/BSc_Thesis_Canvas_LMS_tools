...
// The "$" before the equal sign make this do a postfix match against the string
...
//
// 1st iteration it will find: https://saml-5.sys.kth.se/idp/profile/SAML2/Redirect/SSO?execution=e1s1
// 2nd iteration will find https://login.kth.se/login?service=https%3A%2F%2Fsaml-5.sys.kth.se%2Fidp%2FAuthn%2FExtCas%3Fconversation%3De2s1&entityId=https%3A%2F%2Fwww.diva-portal.org%2Fshibboleth
// 3rd iteration will find https://saml-5.sys.kth.se/idp/profile/SAML2/Redirect/SSO?SAMLRequest=...&RelayState=cookie%3A...
// 4th iteration will find https://kth.test.diva-portal.org/dream/add/add1.jsf
...
async function gotoTargePage(page, partialURL) {
	...
	if (pageFound == target_diva_url) {
	    // once the user is logged in this will produce: FOUND! https://kth.test.diva-portal.org/dream/add/add1.jsf
	    return true;
	}
...
async function main() {
	...
	console.log("request url:", url, "type: ", type);
	request.continue();
    });
	...
	page.on("response", async response => {
	    ...
	    if ((status == 200) && (!(url.endsWith('.jpg') || url.endsWith('.png') ||
				      url.endsWith('.ico')  || url.endsWith('.jsf') ))) {
		try {
		    const buffer= await response.buffer();
		    const s=String(buffer);

		    if (    // image
			s.startsWith('GIF89a') || s.startsWith('GIF87a') ||
			    // document
			    s.includes('<!DOCTYPE html>')  ||
			    // css
			    s.startsWith('article,aside,details') ||
		     	    s.includes('/* CSS Document */') ||
			    s.includes('font-family:') ||
			    // javascript
			    s.startsWith('var ') ||
			    s.startsWith('function') || s.startsWith('(function') ||
			    s.startsWith('.equation,') ||
			    // icon
			    s.startsWith('\u0000\u0000\u0000\u0000') ||
			    s.startsWith('2441\u0000+') ||
			    // tinyMCE
			    s.startsWith('tinyMCE.addI18n') ||
			    s.startsWith('/* Reset */')
		    ) {
		    	console.log(`response buffer content ignored`);
		    	return;
		    }
		    if (s.startsWith('<updates>') ||
			s.startsWith('<html>')  || s.startsWith('<html ')
		       ) {
		    	console.log(`response buffer: ${s.slice(0,100)}`);
		    	return;
		    } else {
		    	console.log(`response buffer: ${s}`);
		    }
		} catch (e) {
		    console.error(`response  url:${url} status: ${status} - failed: ${e}`);
		}
	    }
	});

    // try to naviate to the page where one can add new items to DiVA
    ...
    selector=make_postfix_selector_from_id('publicationType');
    await page.waitFor(selector);
    ...
    // 	return t1;
    // });
    pubtype = await page.$eval(selector, el => el.value);
    console.info("pubtype is initially ", pubtype);

    // set the type of document
    //await page.select('div.diva2addtextplus div.diva2addlistdown2 select.iceSelOneMnu', 'studentThesis');
    await page.select(selector, 'studentThesis');
    await page.reload();
    //await page.waitFor(1000);
    await page.waitFor(selector);
    ...
    selector='div.diva2backfor fieldset.diva2addborder a.diva2linkfor';
	...
	if (textContent === 'Fortsätt') {
		selector = 'div.diva2language div.iceMnuBar div.iceMnuBarItem';
		selector1 = 'div.diva2language div.iceMnuBar div.iceMnuBarSubMenu div.iceMnuItm a.iceCmdLnk';
		try {
			await Promise.all([
				...
				/*	await page.waitFor(selector => {
						selector = 'div.diva2backfor fieldset.diva2addborder a.diva2linkfor';
						if (document.querySelector(selector).textContent === 'Continue') {
							return true;	
						}
						return false;
					}, selector) // wait for page to turn to English*/
				await page.waitFor(2000)
			]);
		} catch (error) {
			console.info("did not successfully click to change language to English")
		}
		selector = 'div.diva2backfor fieldset.diva2addborder a.diva2linkfor';
	}
	...
	await Promise.all([
	    await page.click(selector),
	    await page.waitFor('div.diva2addtextplusname') // wait for the long form to appear
	]);
    } catch (error) {
	console.info("did not successfully click to go to the next page")
    }
    console.log('FOUND after Continue!', page.url()); //  should be https://kth.test.diva-portal.org/dream/add/add2.jsf
    ...
    console.info("Initial areaNames are ", areaNames);
    ...
    // Wait for the Abstract2 to appear
    // initially the 'Another abstract' was 'addForm:j_id1122:0:j_id1148',
    // the addition of the new abstract introduces a new <input> tag:
    // <input class="iceCmdBtn diva2addsokbutton diva2backgroundcolor" id="addForm:j_id1122:1:j_id1148" name="addForm:j_id1122:1:j_id1148" onblur="setFocus('');" onclick="tinyMCE.triggerSave();iceSubmit(form,this,event);return false;" onfocus="setFocus(this.id);" type="submit" value="Another abstract »">
    abstract_button=buttonHandles['Another abstract'];
    new_abstract_button=abstract_button.slice(0,17)+'1';
    selector=make_prefix_selector_from_id(new_abstract_button);
    console.info("selector for new_abstract_button is ", selector);

    try {
	await Promise.race([
	    await page.waitFor(selector), // wait for the 2nd abstract to appear
	    await page.waitFor(2000)    // or wait 2 seconds
	])
    } catch (error) {
	console.info("did not see the 2nd abstract being added")
    }
    ...
    textareaHandles = await getTextAreaHandles(page);
    areaNames = await getTextAreaContent(page);
    console.info("areaNames are now ", areaNames);
...
// 	  <p><strong>Browse:</strong> Click on Browse and locate the file on your computer. Then click on Upload to select the marked file.</p>
    ...
    status = await fileInput.uploadFile(`../utils/${thesis_info['File']['Filename']}`);
    //await page.waitFor(1000);
    await Promise.race([
	pendingXHR.waitForAllXhrFinished(),
	new Promise(resolve => {
